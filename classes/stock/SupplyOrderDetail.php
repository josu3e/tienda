    ...
    public function validateController($htmlentities = true) {
        ...
        if (isset(self::$fieldsRequiredDatabase[get_class($this)])) {
            $fields_required = array_merge(
                    $this->fieldsRequired, self::$fieldsRequiredDatabase[get_class($this)]
            );
        }
        ...
        foreach ($this->fieldsSize as $field => $max_length) {
            if ($value = $this->{$field} && Tools::strlen($value) > $max_length) {
                $errors[] = sprintf(
                        Tools::displayError('%1$s is too long. Maximum length: %2$d'), SupplyOrderDetail::displayFieldName($field, get_class($this), $htmlentities), $max_length
                );
            }
        }
