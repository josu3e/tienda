...
class AdminCartsControllerCore extends AdminController {
    ...
    public function displayAjaxSearchCarts() {
        ...
        if ($orders || $carts) {
            $to_return = array_merge($this->ajaxReturnVars(), array('carts' => $carts,
                'orders' => $orders,
                'found' => true));
        } else {
    ...
    public function ajaxReturnVars() {
        ...
        return array(
            ...
            'link_order' => $this->context->link->getPageLink(
                    'order', false, (int) $this->context->cart->id_lang, 'step=3&recover_cart=' . $id_cart . '&token_cart=' . md5(_COOKIE_KEY_ . 'recover_cart_' . $id_cart)
            ),
