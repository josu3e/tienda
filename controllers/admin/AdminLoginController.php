        ...
        if (!Tools::usingSecureMode() && Configuration::get('PS_SSL_ENABLED')) {
            ...
            if ($clientIsMaintenanceOrLocal) {
                ...
                $warningSslMessage = sprintf(
                        Translate::ppTags(
                                Tools::displayError('SSL is activated. Please connect using the following link to [1]log into secure mode (https://)[/1]', false), array('<a href="%s">')
                        ), $url
                );
            }
    ...
    public function processForgot() {
        if (_PS_MODE_DEMO_) {
            ...
            if (!$employee->getByEmail($email) || !$employee) {
                ...
                $this->errors[] = sprintf(
                        Tools::displayError('You can regenerate your password only every %d minute(s)'), Configuration::get('PS_PASSWD_TIME_BACK')
                );
            }
        }
