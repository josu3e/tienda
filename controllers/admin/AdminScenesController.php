...
class AdminScenesControllerCore extends AdminController {
        ...
        if ($obj->id && (isset($_FILES['image']) || isset($_FILES['thumb']))) {
            ...
            foreach ($images_types as $k => $image_type) {
                if ($image_type['name'] == 'm_scene_default') {
                    ...
                    ImageManager::resize(
                            $base_thumb_path, _PS_SCENE_THUMB_IMG_DIR_ . $obj->id . '-' . stripslashes($image_type['name']) . '.jpg', (int) $image_type['width'], (int) $image_type['height']);
                } elseif (isset($_FILES['image']) && isset($_FILES['image']['tmp_name']) && !$_FILES['image']['error']) {
                    ImageManager::resize(
                            $base_img_path, _PS_SCENE_IMG_DIR_ . $obj->id . '-' . stripslashes($image_type['name']) . '.jpg', (int) $image_type['width'], (int) $image_type['height']);
                }
            }
        }
    ...
    public function initFieldsForm() {
        ...
        $image_to_map_desc .= '<div class="help-block">' . $this->l('Format:') . ' JPG, GIF, PNG. ' . $this->l('File size:') . ' '
                ...
                . sprintf($this->l('If an image is too large, it will be reduced to %1$d x %2$dpx (width x height).'), $large_scene_image_type['width'], $large_scene_image_type['height'])
        ...
        if ($obj->id && file_exists(_PS_SCENE_IMG_DIR_ . $obj->id . '-scene_default.jpg')) {
            ...
            $img_alt_desc .= $this->l('If you want to use a thumbnail other than one generated from simply reducing the mapped image, please upload it here.')
                    ...
                    . sprintf($this->l('Automatically resized to %1$d x %2$dpx (width x height).'), $thumb_scene_image_type['width'], $thumb_scene_image_type['height']) . '.<br />'
