                                    ...
                                    <ul id="ul_layered_{$filter.type}_{$filter.id_key}">
                                        ...
                                        {else}
                                            {if $filter.filter_type == 0}
                                                ...
                                                <script type="text/javascript">
                                                    {literal}
                                                        var filterRange = {/literal}{$filter.max}
                                                            -{$filter.min}{literal} ;
                                            ...
                                            {else}
                                                ...
                                                {else}
                                                    {foreach from=$filter.list_of_values  item=values}
                                                        <li class="nomargin {if $filter.values[1] == $values[1] && $filter.values[0] == $values[0]}layered_list_selected{/if} layered_list" onclick="$('#layered_{$filter.type}_range_min').val({$values[0]});
                                                                $('#layered_{$filter.type}_range_max').val({$values[1]});
                                                                reloadContent();">
